...
export function ProgressBadge({ percentage, className }: ProgressBadgeProps) {
  const getColor = () => {
    ...
    if (percentage >= 50) return "bg-gradient-to-r from-purple-500 to-purple-600";
    ...
    <div className={cn("inline-flex flex-col gap-2", className)}>
      <div className="flex items-center gap-3">
        <div className="flex-1 min-w-[160px] h-3 bg-gray-200 rounded-full overflow-hidden shadow-inner">
          <div
            className={cn("h-full transition-all duration-500 ease-out shadow-sm", getColor())}
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className={cn(
            "text-base font-bold",
            percentage === 100 ? "text-green-600" : "text-gray-700"
          )}>
            {percentage}%
          </span>
        </div>
